...
void lcd_drawline(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2)
{
	...
	{  
		...
		LCD_DrawPoint(uRow, uCol, lcd_device_info.foreColor);/* 画点 */
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void lcd_draw_Circle(unsigned short x0,unsigned short y0,unsigned char r)
{
    ...
    {
        LCD_DrawPoint(x  + mx, y  + my, lcd_device_info.foreColor);
        LCD_DrawPoint(y  + mx, x  + my, lcd_device_info.foreColor);
        LCD_DrawPoint(-x + mx, y  + my, lcd_device_info.foreColor);
        LCD_DrawPoint(-y + mx, x  + my, lcd_device_info.foreColor);
...
{  							  
	...
	{   
		...
		for(t1 = 0; t1 < 8; t1++)
		{			    
			if(temp & 0x80)LCD_DrawPoint(x, y, lcd_device_info.foreColor);
			else if(mode==0)LCD_DrawPoint(x, y, lcd_device_info.backColor);
			temp <<= 1;
			y++;
			if(y >= lcd_device_info.height) return;			/* 超区域了 */	
			if((y - y0) == size)
			{
				y = y0;
				x++;
				if(x >= lcd_device_info.width) return;		/* 超区域了 */
				break;
			}
		}  	 
	}  		    	   	 	  
