...
_start:
    ...
    /**
        ...
        bit0:           0       设置压摆率为低压摆率
        bit5:3          110     设置驱动能力为R0/6，比较强的驱动能力(驱动能力则是输出电流的能力,驱动能力不足或者负载电流过大导致驱动能力不足时可能导致逻辑电路电压不稳定或者不足,从而导致逻辑电路紊乱)
        bit7:6          10      设置速度为中速度
        ...
        bit13           0       设置为使用保持器
        bit15:14        00      100k下拉(将电平钳位在低电平，过滤掉其它可能的干扰电平导致的信号紊乱，因为此时的LED灯需要低电平导通)
        bit16           0       关闭hys
     */
     ldr r1, =0x10B0
     str r1, [r0]
